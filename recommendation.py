...
def get_data():
    ...
    movie_data['genres'].to_numpy()
    movie_data['cast'].to_numpy()
    return movie_data

def combine_data(data):
    data_recommend = data.drop(columns=['budget', 'homepage', 'id', 'keywords', 'original_language', 'original_title', 'overview',
                                        'popularity', 'production_companies', 'production_countries', 'release_date', 'revenue', 'runtime', 'spoken_languages',
                                        'status', 'tagline', 'title', 'vote_average', 'vote_count', 'crew', 'director'])
    data_recommend['combine'] = data_recommend[data_recommend.columns[1:2]].apply(lambda x: ','.join(x.dropna().astype(str)),axis=1)
